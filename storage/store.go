...
// Store wraps a database connection and exposes job-centric helpers.
type Store struct {
	DB  DB
	now func() time.Time
}
...
func NewStore(db DB, nowFn func() time.Time) (*Store, error) {
	if db == nil {
		return nil, fmt.Errorf("database connection is required: %w", apperrors.ErrNotConfigured)
	}
	if nowFn == nil {
		nowFn = time.Now
	}
...
func NewStoreWithProvider(db DB, provider timeprovider.Provider) (*Store, error) {
	if provider == nil {
		provider = timeprovider.RealProvider{}
	}
	return NewStore(db, provider.Now)
}
...
func ScanSchedule(row Row) (ScheduleRow, error) {
	var (
		s         ScheduleRow
		payload   []byte
...
func scanJob(row Row) (Job, error) {
	var (
		job         Job
