...
func (r *Runner) tick(ctx context.Context) error {
	...
	for _, schedule := range schedules {
		...
		res, err := tx.ExecContext(ctx, `UPDATE queue_schedules SET last_enqueued_at=$2 WHERE id=$1 AND (last_enqueued_at IS NOT DISTINCT FROM $3)`, schedule.ID, last, previous)
		if err != nil {
			return err
		}
